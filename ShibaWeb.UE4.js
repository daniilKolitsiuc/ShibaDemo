...
var Module = {
	...
	UE4_indexedDBVersion: 202208211112, // Bump this number to invalidate existing IDB storages in browsers.
};
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('https://dl.dropboxusercontent.com/s/ue8xs98viq0zjn3/UE4Game.wasm?dl=0'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/daniilKolitsiuc/ShibaDemo/d6fb69f48b3535f0ebe9b32bef25344d231fc4c8/UE4Game.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/daniilKolitsiuc/ShibaDemo/52eb786eb3900883019c194c5513a3f0a5d621f0/ShibaWeb.data.js'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/daniilKolitsiuc/ShibaDemo/85378e640049ae4060ead3991a538431e9b47d7a/Utility.js')).then(addScriptToDom);
		var dataDownload =
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('https://www.dropbox.com/s/wf1iyk8ofnx6tks/ShibaWeb.data?dl=0'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('https://www.dropbox.com/s/wf1iyk8ofnx6tks/ShibaWeb.data?dl=0')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
